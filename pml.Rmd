...
### Summary
In this report, a machine learning algorithm "Gradient Boosting Machine (gbm)" is applied on the Human Activity Recognition dataset provided by Groupware. Confusion Matrix Table shows the gbm method scored over 98% accuracy rate. The answer on the 20 test cases is right with 100% accuracy rate.
...
confusionMatrix(result, cv$classe)
mFit
}
```

### 4.Result
Take gbm method for example, 80% of training set is used for training and 20% is used for validation for the out-of-sample accuracy estimate. Gbm method scored 98.5254% accuracy. The whole program took about 42 minutes to finish.
```{r}
library(lattice)
library(ggplot2)
library(splines)
